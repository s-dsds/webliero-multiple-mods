...
function initFirebase() {
    ...
    (async () => {
        ...
        modsRef = fdb.ref(`${baseRoomName}/${CONFIG.room_id}/mods`);
        listenForModsEvents();
...
function loadnewMod(childSnapshot) {
	var v = childSnapshot.val();
	var k = childSnapshot.key;
    if (typeof v.name != undefined && typeof v.url != undefined) {
        try {

            modpool[k] = v;
            shuffleModPool()
            
            console.log("mod `"+v.name+"` `"+v.url+"` has been added");
            notifyAdmins("mod `"+v.name+"` `"+v.url+"` has been added");
        } catch(error) {
            console.log(`error loading mod ${error}`, error.stack);
            notifyAdmins(`error loading mod ${error}`);
        }
     
    }
}

function removeMod(childSnapshot) {
	var k = childSnapshot.key;
	var n = modpool[k];
    delete modpool[k];
    shuffleModPool();
